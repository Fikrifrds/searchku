...
import { Search as SearchIcon, Book, X, ExternalLink, ChevronLeft, ChevronRight, Languages } from 'lucide-react';
...
export default function Search() {
  ...
  const handleBookFilterChange = (bookId) => {
    ...
    // Trigger new search with updated filter if there's an active search
    if (query.trim()) {
      handleSearch(1);
    }
  };
  ...
  const totalPages = Math.max(1, Math.ceil((totalResults || 0) / resultsPerPage));

  // Pagination handlers
  const handlePageChange = (pageNumber) => {
    const safeTotalPages = Math.max(1, totalPages);
    if (pageNumber >= 1 && pageNumber <= safeTotalPages && !isSearching) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Pagination */}
      {searchResults.length > 0 && totalResults > 0 && totalPages > 1 && (
        <div className="bg-white shadow rounded-lg p-6">
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="flex items-center space-x-1">
                ...
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                  ...
                  if (pageNum < 1 || pageNum > safeTotalPages || isNaN(pageNum)) return null;
                ...
                {(() => {
                  const safeCurrentPage = Math.max(1, Math.min(currentPage || 1, totalPages));
                  const safeTotalPages = Math.max(1, totalPages);
                  return safeCurrentPage < safeTotalPages - 2 && (
                    <>
...
function SearchResultCard({ result, navigate }) {
  ...
  useModalScrollPrevention(showModal);
  const {
    page_id,
    book_id,
    page_number,
    ...
    <div className="border border-gray-200 rounded-lg overflow-hidden hover:shadow-md transition-shadow bg-white">
      ...
      <div className="p-4 border-b border-gray-100">
        ...
        <div className="mt-3 flex items-center justify-between">
          <div className="flex items-center space-x-2">
            {page_image_url && (
              <button
                ...
                className="inline-flex items-center px-3 py-1.5 text-sm font-medium text-blue-700 bg-blue-50 border border-blue-200 rounded-lg hover:bg-blue-100 hover:border-blue-300 transition-colors duration-200"
              >
            ...
            <button
              ...
              className="inline-flex items-center space-x-1.5 px-3 py-1.5 text-sm font-medium text-green-700 bg-green-50 border border-green-200 rounded-lg hover:bg-green-100 hover:border-green-300 transition-colors duration-200"
            >
          ...
          {id_translation && (
            <div className="flex items-center space-x-1 px-2 py-1 bg-purple-50 border border-purple-200 rounded-lg">
              <Languages className="w-4 h-4 text-purple-600" />
              <span className="text-xs font-medium text-purple-700">Translation</span>
            </div>
          )}
        </div>
      </div>
